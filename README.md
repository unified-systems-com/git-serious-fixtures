...
| `.github/workflows/secret-reach-in-scope.yml` | `${{ secrets.TAP_SCOPE_PROBE }}` — an organisation secret whose `visibility` is `selected`, with **this repository on the list**. | The reference RESOLVES: a `REFERENCES_SECRET` edge from this workflow to the `actions_secret` node, and the name absent from `tags.secret_refs.unresolved`. Proves the collector follows `selected_repositories_url` rather than guessing (`github-core#107`). |
| `.github/workflows/secret-reach-out-of-scope.yml` | `${{ secrets.TAP_SCOPE_PROBE_ELSEWHERE }}` — an organisation secret whose `visibility` is `selected`, with **this repository NOT on the list**. | The reference is UNRESOLVED and gets **no edge**. The secret exists, so this is not the same fact as a missing name — but at runtime GitHub hands the step an empty string either way, which is why both land in `unresolved`. The shape that read as a working reference before `github-core#107`. |
